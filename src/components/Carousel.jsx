...
import Banner8 from "@assets/desconexionDigital.jpg";
...
export default function CarouselSection() {
  ...
  const renderArrowPrev = (onClickHandler, hasPrev, label) =>
    hasPrev && (
      <button
        ...
        className="absolute z-10 max-w-8 top-0 bottom-0 cursor-pointer text-white/60 hover:text-white transition-colors ml-3"
      >
        <svg
          width="100%"
          height="auto"
  ...
  const renderArrowProx = (onClickHandler, hasNext, label) =>
    hasNext && (
      <button
        onClick={onClickHandler}
        className="absolute z-10 max-w-8 top-0 bottom-0 right-0 cursor-pointer text-white/60 hover:text-white transition-colors mr-3"
      >
        <svg
          width="100%"
          height="100%"
